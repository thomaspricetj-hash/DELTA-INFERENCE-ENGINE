// delta_inference/src/python_bindings.rs
use pyo3::prelude::*;
...
/// Helper to build StateId from Python args
...
fn delta_bindings(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    fn py_compute_delta_f32(
        ...
        match compute_delta_f32(id, master, variant, tolerance) {
            Ok(delta) => serde_json::to_string(&delta).map_err(|e| PyValueError::new_err(e.to_string())),
            Err(e) => Err(PyValueError::new_err(e.to_string())),
        }
    }

    #[pyfn(m, "apply_delta_f32")]
    fn py_apply_delta_f32(
        py: Python,
        master: PyReadonlyArray1<f32>,
        delta_json: String,
    ) -> PyResult<PyObject> {
